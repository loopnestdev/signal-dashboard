...
- Sectors above 50d MA count --> +(count/total x 20) rounded
- Clamp 0-100
...
**computeMarketQualityScore**: `volx0.20 + trendx0.25 + breadthx0.20 + momentumx0.25 + macrox0.10`, rounded
...
- `ScoreRing` sub-component: SVG circle with animated `stroke-dashoffset` from 0 to (score/100xcircumference); score number + "/ 100" label centered; ring color: >=80-->green, >=60-->yellow, else red; `filter: drop-shadow` glow on the arc
...
- Row: category label + weight badge | score/100 (muted) | contribution (weightxscore/100 in color) | animated bar
...
- **During market hours:** gives today's intraday change [OK]
